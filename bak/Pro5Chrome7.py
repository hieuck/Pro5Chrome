...
def login_google(profile):
    ...
    profile_directory = f"--profile-directory=Profile {profile}"
    subprocess.Popen([chrome_path_entry.get(), profile_directory, login_url])
...
def choose_chrome_path():
    ...
    if chrome_path:
        ...
        chrome_path_entry.insert(0, chrome_path)
...
# Đường dẫn Chrome
chrome_path_frame = ttk.Frame(root)
chrome_path_frame.pack(pady=10)

chrome_path_label = ttk.Label(chrome_path_frame, text="Đường dẫn Chrome:")
chrome_path_label.pack(side=tk.LEFT, padx=5)

chrome_path_entry = ttk.Entry(chrome_path_frame, width=50)
chrome_path_entry.pack(side=tk.LEFT, padx=5)

browse_button = ttk.Button(chrome_path_frame, text="Chọn", command=choose_chrome_path)
