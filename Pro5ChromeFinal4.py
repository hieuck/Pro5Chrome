...
url_label = ttk.Label(root, text="Nhập URL mới:")
url_label.pack()

url_entry = ttk.Entry(root, width=50)
url_entry.pack()

# Hàm xử lý khi nhấn nút để lưu URL mới vào danh sách và cập nhật giao diện
def save_new_url():
    new_url = url_entry.get()
    if new_url:
        save_url_to_list_and_file(new_url)
        url_entry.delete(0, tk.END)  # Xóa nội dung trong entry sau khi lưu
    else:
        print("Vui lòng nhập URL")

# Nút để lưu URL mới
save_url_button = ttk.Button(root, text="Lưu URL mới", command=save_new_url)
...
# Nút để xóa các URLs đã chọn
delete_urls_button = ttk.Button(root, text="Xóa URL đã chọn", command=delete_selected_urls)
