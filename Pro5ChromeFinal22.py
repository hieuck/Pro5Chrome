...
# Nút Lưu URL và mở trong Chrome
def save_url_and_open_in_chrome():
    url = url_entry.get().strip()
    if url:
        save_url_to_list_and_file(url)
        open_url(url)
        url_entry.delete(0, tk.END)
    else:
        print("Vui lòng nhập một URL")

save_button = ttk.Button(root, text="Lưu URL và Mở Chrome", command=save_url_and_open_in_chrome)
save_button.pack(pady=10)

# Hiển thị danh sách các URL đã lưu
urls_label = ttk.Label(root, text="Danh sách URL đã lưu:")
urls_label.pack()
